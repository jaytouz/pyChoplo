...
def create_df_single_col(moves, c_name, lvl_num, n_lvl=4, n_p=10, n_m=30):


    col_move_info = [c_name, 'lvl_{}'.format(lvl_num), 'side', 'target_reach', 'have_cm'] #col to be used for group_by
    col_cm_metric = ['start_cm_idx', 'start_cm_val', 'rel_pt_idx', 'rel_pt_val', 'end_cm_idx', 'end_cm_val', 'abs_1', 'abs_2', 'abs_tot']

    col_tml_metric = ['max_vel_idx', 'max_vel_val', 'max_amp_idx', 'max_amp_val', 'time_tml', 'd_shift',
                      'rel_pt_to_max_amp', 'overshoot', 'overshoot_time', 'd_nmp', 'r_nmp_shift']
    col_target_metric = ['tm_target_p', 'tm_target_time', 'dis_target_p']
    col_dis_pourc = ['dis_5p_time', 'dis_10p_time', 'dis_25p_time', 'dis_50p_time', 'dis_75p_time']
    col_tm_pourc = ['tm_5p_time', 'tm_10p_time', 'tm_25p_time', 'tm_50p_time','tm_75p_time']

    all_cols = []
    all_cols.extend(col_move_info)
    all_cols.extend(col_cm_metric)
    all_cols.extend(col_tml_metric)
    all_cols.extend(col_target_metric)
    all_cols.extend(col_dis_pourc)
    all_cols.extend(col_tm_pourc)

    n_row = n_lvl * n_p * n_m

    df = pd.DataFrame(index=n_row, columns=all_cols)
    i = 0
    for m in multisenso:
